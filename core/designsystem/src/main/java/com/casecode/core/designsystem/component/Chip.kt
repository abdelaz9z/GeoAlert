...
/**
 * GeoAlert filter chip with included leading checked icon as well as text content slot.
 *
...
fun GeoAlertFilterChip(
    ...
    FilterChip(
        ...
        leadingIcon = if (selected) {
            {
                Icon(
                    imageVector = GeoAlertIcons.Check,
        ...
        border = FilterChipDefaults.filterChipBorder(
            ...
            disabledBorderColor = MaterialTheme.colorScheme.onBackground.copy(
                alpha = GeoAlertChipDefaults.DISABLED_CHIP_CONTENT_ALPHA,
            ),
            disabledSelectedBorderColor = MaterialTheme.colorScheme.onBackground.copy(
                alpha = GeoAlertChipDefaults.DISABLED_CHIP_CONTENT_ALPHA,
            ),
            selectedBorderWidth = GeoAlertChipDefaults.ChipBorderWidth,
        ),
        colors = FilterChipDefaults.filterChipColors(
            ...
            disabledContainerColor = if (selected) {
                MaterialTheme.colorScheme.onBackground.copy(
                    alpha = GeoAlertChipDefaults.DISABLED_CHIP_CONTAINER_ALPHA,
                )
            } else {
                Color.Transparent
            },
...
fun ChipPreview() {
    GeoAlertTheme {
        GeoAlertBackground(modifier = Modifier.size(80.dp, 20.dp)) {
            GeoAlertFilterChip(selected = true, onSelectedChange = {}) {
                Text("Chip")
            }
        }
    }
